...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (opcao == 1) {
			...
			System.out.println("                                                           ");
			System.out.println(
					"Seja Bem-vinde, " + nome + "!\rBom, agora que sabemos quem você é, bora começar a nossa SAGA...");
			System.out.println("															");
		} else if (opcao == 2) {
			System.out.println("                                             ");
			System.out.println("\nSaindo do jogo....");
		...
		dadosJogador[][] perguntas = { { new dadosJogador(
				...
				new dadosJogador(
						...
						3) }, // Fase 2
				{ new dadosJogador(
						"Você ta mandando bem demais.\r\r\t\t\t''A motivação é uma porta que se abre por dentro.''\r\rResponda: Quais são os principais laços condicionais em lógica de programação? ",
				...
				{ new dadosJogador(
						...
						new dadosJogador(
								"Estmaos gostando de ver.\r\r\t\t\t''O sucesso começa quando a motivação termina e a disciplina continua.''\r\rResponda: Qual a diferença entre o laço for e o laço while em programação?",
				...
				{ new dadosJogador(
						...
						2),
						new dadosJogador(
								"''O fraco reclama e fica inerte, o guerreiro faz e transforma.''\r\r\t\t\t\r\rResponda: Em uma Array do tipo String qual é a maneira correta de se usar a Interface Iterator?",
								new String[] { "Iterator<T> nomeDoIterator = nomeDaCollection.iterator();\r\n" + "",
										"Iterator<G> nomeDoIterator = nomeDoIterator.iterator();\rIterator<T> nomeDaCollection = nomeDoIerator.iterator();\r\n"
												+ "Iterator<T> nomeDaCollection = nomeDoIerator.iterator();\r\n" + "" },
								1) }, // Fase 6
				{ new dadosJogador(
						"Você prestes a finalizar a SAGA. Chegue ao Céu!!!\r\r\t\t\t\r\rResponda: Quais são os modificadores de acesso de métodos e atributos?",
		...
		for (int x = 0; x < 6; x++) {
			...
			for (int i = 0; i < 2; i++) {
				...
				if (resposta == perguntaAtual.getRespostaCorreta()) {
					pontuacao += 10;
					System.out
							.println("Muito bem, Resposta certa!\rSua pontuação atual é de " + pontuacao + " pontos.");
					System.out.println("                                                                   ");
					System.out.println("                                                                   ");
